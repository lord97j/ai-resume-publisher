...
Encrypted full resumes are unlocked with the `#key=...` fragment generated by `npm run encrypt`. Do not publish real private keys in public README files.
...
```bash
npm run encrypt -- --key "user-chosen-key"
```

Key source priority is `--key`, `RESUME_PRIVATE_KEY`, private `resume.json -> publisher.encryption.key`, existing ignored `public/decrypt.key`, then a generated random key. For GitHub Actions, prefer a repository secret named `RESUME_PRIVATE_KEY`; only use `publisher.encryption.key` in private repositories.
...
| Custom key gets lost | Store it as the private repository secret `RESUME_PRIVATE_KEY`, or rerun `npm run encrypt -- --key "<key>"` locally |
